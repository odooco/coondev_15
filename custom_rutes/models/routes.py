...
class Routes(models.TransientModel):
    ...
    activity_id = fields.Many2many('route.activity', 'enroute_activity_rel', 'enroutes_id', 'activity_id', string='Actividad de La Ruta', default=lambda self: self._default_activity(), domain=lambda self: [("id", "in", self._get_activity_id_domain())])
...
class Routes(models.Model):
    ...
    domiciliary_id = fields.Many2one('res.partner.domiciliary', string='Domiciliario', tracking=True, group_expand='_expand_domiciliary')
    activity_id = fields.Many2many('route.activity', 'route_activity_rel', 'routes_id', 'activity_id', string='Actividad de La Ruta', default=lambda self: self._default_activity())
    rute_comment = fields.Text(string='Comentarios de Enrutado')
    state = fields.Selection([('draft', 'Creada'), ('route', 'En Ruta'), ('post', 'Realizada'), ('not_post', 'No Realizada'), ('refuse', 'Rechazada'), ('copy', 'Duplicada'), ('cancel', 'Cancelada')], string='Estado', required=True, default='draft')
    ...
    weekly_schedule_am = fields.Many2many('calendar.day.megatoner', 'r_day_weekly_am_rel', 'routes_id', 'calendar_day_id', string='Weekly Schedule Am')
    weekly_schedule_pm = fields.Many2many('calendar.day.megatoner', 'r_day_weekly_pm_rel', 'routes_id', 'calendar_day_id', string='Weekly Schedule Am')
    saturday_schedule_am = fields.Many2many('calendar.day.megatoner', 'r_day_saturday_am_rel', 'routes_id', 'calendar_day_id', string='Saturday Schedule AM')
    saturday_schedule_pm = fields.Many2many('calendar.day.megatoner', 'r_day_saturday_pm_rel', 'routes_id', 'calendar_day_id', string='Saturday Schedule AM')
    origin = fields.Selection([('manual', 'Rutas'), ('s_order', 'Orden de Venta'), ('p_order', 'Orden de Compra'), ('contact', 'Contacto'), ('ticket', 'Ticket'), ('piking', 'Entrega'), ('invoice', 'Factura')], string='Origen')
