...
class AccountInvoicePurchase(models.TransientModel):
    _name = 'account.invoice.purchase'
    _descripcion = 'factura de proveedor'

    def _get_default_journal(self):
        return self.env['account.move']._search_default_journal(('puchase'))
                ...
                for invoice_line in invoice_data['invoice_lines']:
                    ...
                    if 'taxes' in invoice_line:
                        for invoice_line_tax in invoice_line['taxes']:
                            ...
                            if not tax:
                                raise ValidationError(
                                    'No se encontró el impuesto indicado en el XML de la factura,'
                                    'por favor verificar que esté parametrizado en odoo con el código DIAN= {} y el porcentaje {}% '
                                    'indicado en la factura, como impuesto de Compras.'.format(invoice_line_tax['code'], invoice_line_tax['percentage']))
